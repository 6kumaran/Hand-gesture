...
def main():
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                debug_image = draw_info_text(
                    ...
                    brect,
                    handedness,
                    keypoint_classifier_labels[hand_sign_id],#This one
                    point_history_classifier_labels[most_common_fg_id[0][0]],
                )
                l.append(keypoint_classifier_labels[hand_sign_id])
        else:
            point_history.append([0, 0])
    ...
    for i in l:
        if i not in l2:
            l2.append(i)
    for j in l2:#Now have to remove continuously repeating words from string or list before forming that into a string
        s=s+" "+j
    print(s)
    txt_spch=pyttsx3.init()
    ans="The Person is Saying "+s
    txt_spch.say(ans)
    txt_spch.runAndWait()
